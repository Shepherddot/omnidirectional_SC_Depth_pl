...
class SC_DepthV2(LightningModule):
    ...
    def setup(self, stage):
        ...
        if self.hparams.hparams.val_mode == 'depth':
            self.val_dataset = ValidationSet(self.hparams.hparams.dataset_dir, transform=valid_transform, dataset=self.hparams.hparams.dataset_name)
    ...
    def rectify_imgs(self, tgt_img, ref_imgs, calib_cam):
        ...
        for ref_img in ref_imgs:
            ...
            rot_warped_img = inverse_rotation_warp(ref_img, rot1, calib_cam)
            rot1_list += [rot1]
    ...
    def forward(self, tgt_img, ref_imgs, calib_cam):
        # in lightning, forward defines the prediction/inference actions for training
        tgt_depth = self.inference_depth(tgt_img)
        ref_imgs_warped, loss_rc, loss_rt, rot_before, rot_after = self.rectify_imgs(tgt_img, ref_imgs, calib_cam)
        ...
        # poses_inv = [self.inference_pose(im, tgt_img) for im in ref_imgs_warped]

        return tgt_depth, ref_depths, ref_imgs_warped, loss_rc, loss_rt, rot_before, rot_after
    ...
    def training_step(self, batch, batch_idx):
        ...
        tgt_img, ref_imgs, poses_Trt, poses_Ttr = batch

        # network forward
        tgt_depth, ref_depths, ref_imgs_warped, loss_rc, loss_rt, rot_before, rot_after = self(tgt_img, ref_imgs, self.calib_cam)
        ...
        loss_1, loss_2 = photo_and_geometry_loss(tgt_img, ref_imgs_warped, tgt_depth, ref_depths, self.calib_cam, poses, poses_inv, self.hparams.hparams)
    ...
    def validation_step(self, batch, batch_idx):
        if self.hparams.hparams.val_mode == 'depth':
            ...
            poses = poses_Trt
            poses_inv = poses_Ttr
            tgt_depth, ref_depths, *other = self(tgt_img, ref_imgs, self.calib_cam)
            loss_1, loss_2 = photo_and_geometry_loss(tgt_img, ref_imgs, tgt_depth, ref_depths, self.calib_cam, poses, poses_inv, self.hparams.hparams)
