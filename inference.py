...
def main():
    ...
    if hparams.model_version == 'v1':
        system = SC_Depth(hparams, model_path=None, is_train=False)
    ...
    output_dir = Path(hparams.output_dir) / 'model_{}'.format(hparams.model_version)
    ...
    image_files = sum([(input_dir).files('*.{}'.format(ext)) for ext in ['jpg', 'png']], [])
    ...
    for i, img_file in enumerate(tqdm(image_files)):
        ...
        tensor_img = inference_transform([img])[0].unsqueeze(0).cuda(7)
        pred_depth = model.inference_depth(tensor_img)

        if hparams.save_vis:
            # print(pred_depth.shape)
            # print('save img: ' + output_dir / 'vis/{}.jpg'.format(filename))
            vis = visualize_inference(pred_depth[0, 0], system.origin_mask)[0].permute(1, 2, 0).numpy() * 255
